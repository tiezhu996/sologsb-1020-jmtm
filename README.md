...
- 重复导入修订表时按「分组 + 编号」对应原记录，只写入有差异的字段，并在记录旁保留改前改后；有变更的匹配退回待复核，无变化的结论沿用，新编号自动新增；同一文件内编号重复时整份拒绝导入并指出重复位置。
...
记录和审计轨迹保存在当前浏览器。再次导入同一批记录的修订表时，系统按分组与编号对应原记录：仅写入有差异的字段并保留改前改后，涉及变更的匹配退回待复核，无变化的确认、忽略与合并结论继续沿用，新编号作为新记录加入。同一文件内出现重复编号时，整份修订不会进入工作台，并会指出重复所在的行。
